...
def detect_istio_version(context: Optional[str] = None) -> str:
    cmd = ["kubectl", "get", "deployment", "istiod", "-n", "istio-system", "-o", "jsonpath={.spec.template.spec.containers[0].image}"]
    if context:
    ...
    try:
        ...
        raise RuntimeError("Failed to detect istiod deployment in istio-system namespace. Make sure kubectl is configured.")
