...
def test_get_available_versions_limit(monkeypatch):
    mock_get = MagicMock()
    mock_get.return_value.json.return_value = [{"tag_name": f"1.{i}", "prerelease": False, "draft": False} for i in range(5)]
    monkeypatch.setattr("httpx.get", mock_get)
...
def test_install_version_full(mock_home, mock_os_arch, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("istioswitch.installer.get_asset_name", lambda x: "istio-1.0.tar.gz")
    monkeypatch.setattr("istioswitch.installer.download_file", lambda url, dest: dest.write_bytes(b"archive"))
    monkeypatch.setattr("istioswitch.installer.fetch_expected_checksum", lambda v, a: "fakehash")
    monkeypatch.setattr("istioswitch.installer.verify_checksum", lambda path, hash: True)
...
def test_install_version_checksum_fail(mock_home, mock_os_arch, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("istioswitch.installer.download_file", lambda url, dest: dest.write_bytes(b"archive"))
    monkeypatch.setattr("istioswitch.installer.fetch_expected_checksum", lambda v, a: "fakehash")
    monkeypatch.setattr("istioswitch.installer.verify_checksum", lambda path, hash: False)
