...
def test_list_command(runner, monkeypatch):
    monkeypatch.setattr("istioswitch.installer.get_available_versions", lambda x: ["1.0", "1.1"])
...
def test_use_command_not_in_path_win(runner, monkeypatch):
    monkeypatch.setattr("istioswitch.installer.is_installed", lambda x: True)
    monkeypatch.setattr("istioswitch.switcher.use_version", lambda x: (False, "/fake/bin"))
...
def test_use_command_error(runner, monkeypatch):
    monkeypatch.setattr("istioswitch.installer.is_installed", lambda x: True)
    def mock_use(*args): raise RuntimeError("Use error")
...
def test_install_command_error(runner, monkeypatch):
    monkeypatch.setattr("istioswitch.installer.is_installed", lambda x: False)
    def mock_install(*args): raise RuntimeError("Install error")
...
def test_detect_command_error(runner, monkeypatch):
    def mock_detect(*args): raise RuntimeError("Detect error")
...
def test_uninstall_command_error(runner, monkeypatch):
    def mock_uninstall(*args): raise RuntimeError("Uninstall error")
